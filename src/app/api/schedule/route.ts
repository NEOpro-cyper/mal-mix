...
import { getAnimeBasic } from '@/lib/jikan';
...
// Broadcast times are JST (UTC+9). We compute the next airing time in UTC.
...
//
// PRIMARY: Jikan GET /anime/{id} — status + broadcast
//   broadcast: { day: "Sundays", time: "01:58", timezone, string }
// FALLBACK: official MAL API v2 GET /anime/{id}?fields=status,broadcast
//   broadcast: { day_of_the_week: "sunday", start_time: "01:35" }
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      const anime = await getAnimeBasic(resolved.malId);
      const airing = anime.status === 'Currently Airing';
      const b = (anime.broadcast || null) as { day?: string; time?: string } | null;

      if (airing && b?.day && b?.time) {
        // "Sundays" → "sunday" (strip plural 's', lowercase)
        const dayKey = b.day.toLowerCase().replace(/s$/, '');
        const dayOfWeek = DAY_INDEX[dayKey];
      ...
      console.warn(
        '[schedule] Jikan failed, falling back to official MAL API:',
