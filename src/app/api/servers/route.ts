...
import { resolveAnilistId, getCached, setCache } from '@/lib/jikan';
...
// cross-references, so we resolve MAL→AniList via Jikan's
// /anime/{id}/external endpoint (AniList link in MAL's external-links
// sidebar). Result is cached for 30 days.
export async function GET(request: NextRequest) {
  ...
  // If we only have a MAL ID, resolve it to AniList ID via Jikan /external
  if (!anilistId && malId !== null) {
